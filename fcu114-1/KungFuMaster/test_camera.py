...
from helper.model import pose_model, get_pose_model, YOLO11_POSE_MODELS, DEFAULT_POSE_MODEL
...
def main():
    ...
    # 顯示 YOLOv11 姿態模型資訊
    model_name = Path(posture_detector.model_path).stem
    print(f"\n[姿態估計模型]")
    print(f"  模型: {model_name}")
    print(f"  任務: {posture_detector.model.task}")
    print(f"  可用模型大小: {list(YOLO11_POSE_MODELS.keys())}")

    # 顯示分類模型資訊
    info = kungfu_classifier.get_model_info()
    print(f"\n[動作分類模型]")
    print(f"  類型: {info['model_type']}")
